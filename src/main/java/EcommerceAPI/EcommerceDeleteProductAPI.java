...
public class EcommerceDeleteProductAPI {
    ...
    public static void main(String args[]) {
        ...
        final String viewOrderResourceURI = "api/ecom/order/get-orders-details";
        final String deleteOrderResourceURI = "api/ecom/order/delete-order/";
        final String deleteProductResourceURI = "api/ecom/product/delete-product/";
        ...
        File img = new File("src/main/resources/images.png");
        ...
        String productOrderedId = js2.getString("productOrderId[0]");
        System.out.println("<-- ORDER ID --> "+orderId);
        System.out.println("<-- PRODUCT ID --> "+productOrderedId);

        //View Order Details

        RequestSpecification requestBodyViewOrder = given()
                .log().all()
                .queryParams("id",orderId)
                .header("Authorization",session_token)
                .spec(req);

        Response responseBodyViewOrder = requestBodyViewOrder
                .when().get(viewOrderResourceURI)
                .then().log().all().spec(res)
                .extract().response();

        String responseBodyViewOrderString = responseBodyViewOrder.asString();
        System.out.println(responseBodyViewOrderString);

        //Delete Order API

        RequestSpecification requestBodyDeleteOrder = given()
                .log().all()
                .header("Authorization",session_token)
                .spec(req);

        Response responseBodyDeleteOrder = requestBodyDeleteOrder
                .when().delete(deleteOrderResourceURI+orderId)
                .then().log().all().spec(res)
                .extract().response();

        String responseBodyDeleteOrderString = responseBodyDeleteOrder.asString();
        System.out.println(responseBodyDeleteOrderString);

        //Delete Product API

        RequestSpecification requestBodyDeleteProduct = given()
                .log().all()
                .header("Authorization",session_token)
                .spec(req);

        Response responseBodyDeleteProduct = requestBodyDeleteProduct
                .when().delete(deleteProductResourceURI+productOrderedId)
                .then().log().all().spec(res)
                .extract().response();

        String responseBodyDeleteProductString = responseBodyDeleteProduct.asString();
        System.out.println(responseBodyDeleteProductString);
    }
